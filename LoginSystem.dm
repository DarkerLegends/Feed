...
mob/new_client
	verb
		register_to_server()
			set hidden = 1
			var/user = winget(src, "UsernameIn", "text")
			var/pass = winget(src, "PasswordIn", "text")
			if(!user || !length(user))
				winset(src,null,"log_reg_window.error.text=\"No username entered.\"");return
			if(!pass || !length(pass))
				winset(src,null,"log_reg_window.error.text=\"No password entered.\"");return
			if(user in global.accounts)
				winset(src,null,"log_reg_window.error.text=\"This username in use!\"");return
		...
		login_to_server()
			...
			for(var/check = 0 to length(global.accounts))
				...
				if(user == global.accounts[counter])
					...
					break
			if(!found)
				winset(src,null,"log_reg_window.error.text=\"Username not found.\"");return
			if(!(pass == global.accountspass[counter]))
				winset(src,null,"log_reg_window.error.text=\"Wrong Password.\"");return
